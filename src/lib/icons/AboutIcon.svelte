<script lang="ts">
  export let filled: boolean = false;
</script>

<svg
  width="24"
  height="24"
  viewBox="0 0 24 24"
  fill="none"
  xmlns="http://www.w3.org/2000/svg"
>
  {#if filled}
    <g clip-path="url(#clip0_1_186)">
      <path
        d="M7 20H11C11 21.1 10.1 22 9 22C7.9 22 7 21.1 7 20ZM5 19H13V17H5V19ZM16.5 9.5C16.5 13.32 13.84 15.36 12.73 16H5.27C4.16 15.36 1.5 13.32 1.5 9.5C1.5 5.36 4.86 2 9 2C13.14 2 16.5 5.36 16.5 9.5Z"
        fill="#370520"
      />
      <path
        d="M20 8L21.37 7.37L22 6L22.63 7.37L24 8L22.63 8.63L22 10L21.37 8.63L20 8Z"
        fill="#F2107D"
      />
      <path
        d="M19.94 3.94L19 6L18.06 3.94L16 3L18.06 2.06L19 0L19.94 2.06L22 3L19.94 3.94Z"
        fill="#F2107D"
      />
    </g>
    <defs>
      <clipPath id="clip0_1_186">
        <rect width="24" height="24" fill="white" />
      </clipPath>
    </defs>
  {:else}
    <g clip-path="url(#clip0_1_24)">
      <path
        d="M7 20H11C11 21.1 10.1 22 9 22C7.9 22 7 21.1 7 20ZM5 19H13V17H5V19ZM16.5 9.5C16.5 13.32 13.84 15.36 12.73 16H5.27C4.16 15.36 1.5 13.32 1.5 9.5C1.5 5.36 4.86 2 9 2C13.14 2 16.5 5.36 16.5 9.5ZM14.5 9.5C14.5 6.47 12.03 4 9 4C5.97 4 3.5 6.47 3.5 9.5C3.5 11.97 4.99 13.39 5.85 14H12.15C13.01 13.39 14.5 11.97 14.5 9.5ZM21.37 7.37L20 8L21.37 8.63L22 10L22.63 8.63L24 8L22.63 7.37L22 6L21.37 7.37ZM19 6L19.94 3.94L22 3L19.94 2.06L19 0L18.06 2.06L16 3L18.06 3.94L19 6Z"
        fill="black"
      />
    </g>
    <defs>
      <clipPath id="clip0_1_24">
        <rect width="24" height="24" fill="white" />
      </clipPath>
    </defs>
  {/if}
</svg>
